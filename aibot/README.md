...
Адамдар ботқа жазады → бот AI арқылы **сурет / суреттерді біріктіру / суретті жандандыру / 🎵 ән (вокал + музыка)** жасап береді.
...
| `LIMIT_MUSIC` | 1 | 🎵 Тәуліктік тегін ән лимиті (Replicate ақылы — аз ұстаңыз) |
| `MODEL_MUSIC` | `lucataco/ace-step` | Мәтіннен толық ән (вокал+музыка). ACE-Step — үлгі видео керек етпейді |
| `MUSIC_TAGS_FIELD` | `tags` | Музыка моделінің **стиль** өрісі (жанр/көңіл-күй/вокал) |
| `MUSIC_LYRICS_FIELD` | `lyrics` | Музыка моделінің **ән мәтіні** өрісі |
| `MUSIC_DURATION_FIELD` | `duration` | Ұзақтық өрісінің аты (бос болса — жіберілмейді) |
| `MUSIC_DURATION` | 60 | Ән ұзақтығы (секунд) |
...
### 🎵 «Ән жасау» (music) туралы — авто пилот

Пайдаланушы тек **қарапайым идея** жазады (мыс. «ауыл туралы көңілді ән»).
Сосын бот **бәрін өзі істейді** (авто пилот):
1. Тегін LLM (Pollinations) идеядан **толық ән мәтінін** жазады — қолданушы
   тілінде (қазақша идея → қазақша ән), `[verse]`/`[chorus]` құрылымымен;
2. әнге сәйкес **стиль тегтерін** (жанр/көңіл-күй/вокал) таңдайды;
3. бұларды музыка моделіне беріп, **вокалмен толық ән** жасайды;
4. нәтижені аудио файл етіп, ән мәтінімен бірге жібереді.

Әдепкі модель — `lucataco/ace-step` (мәтін → вокалды ән, **үлгі видео/аудио
қажет емес** — сондықтан авто пилотқа ыңғайлы). Бұл режим **тек Replicate**
арқылы істейді (тегін нұсқасы жоқ), сондықтан тәуліктік тегін лимит `LIMIT_MUSIC`
**аз** (әдепкі 1) — зиянды бақылау үшін. Жазылым алғандарға — шексіз.

> ⚠️ Suno/Udio-ның ресми ашық API-і жоқ — оларды ботқа тікелей қосу мүмкін емес.
> ACE-Step — нәтижесі соларға жақын, API-мен жұмыс істейтін ашық модель. Модель
> схемасы өзгерсе, `MODEL_MUSIC` / `MUSIC_*_FIELD` айнымалыларын реттеңіз.
