...
2. **Лимит бітсе** — «💎 Сатып алу» батырмасы шығады. Пайдаланушы **Telegram Stars**
   арқылы пакет сатып алады (банк-эквайринг қажет емес, KZ-да жұмыс істейді).
   Пакеттер `config.py` ішіндегі `PACKAGES`-те (әдепкі: 10/50⭐, 50/200⭐, 250/750⭐).
3. **Кредит** — төлемнен кейін генерация саны есепке қосылады; лимиттен тыс
   жұмыста сол кредиттен жұмсалады.
4. **Маржа** — генерация шығыны (сурет тегін, Replicate видео ~қымбат) мен
   Stars бағасының айырмасы. Stars-ты Telegram арқылы ақшаға шығаруға болады.

> Төлем коды толық қосылған: `aiogram` Telegram Stars (`XTR` валютасы,
> `answer_invoice` + `pre_checkout` + `successful_payment`). Пакет бағасын
> `PACKAGES`-тен өзгертіңіз.
